...
with open('mag_observables.pickle', 'rb') as f:
    h_f, l_f, u_f, alphak_f, tau_f, biso_f, bani_f, Bbar_f, tanpB_f, tanpb_f = pickle.load(
        f)
...
#               (1e+6*365*24*60*60), c='g', markersize=1.2, linestyle='--', marker='o', mfc='k', mec='k', label=r'$\gamma$')
# ax[i][j].set_xlabel('Radius(kpc)')
# ax[i][j].set_ylabel(r'local e-folding time $\gamma$ (Myr)', size='large')
# ax[i][j].legend()
# fig.suptitle(r'$T_{avg} = $ '+str(T.mean()), fontsize = 15)
params = {}
