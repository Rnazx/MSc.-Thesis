...
# data = zip(kpc_r_Claude, Msunpc2_SigmaTot)

# file_name='sigmatot_Claude_M31.csv'
...
#     writer.writerow(['r kpc', 'sigma_tot'])  # Write header row
...
if __name__ == '__main__':
    ...
    # dat_sigmatot = griddata(kpc_r_cl, Msunpc2_SigmaTot*g_Msun/(cm_pc)** 2, kpc_r, method='linear', fill_value=nan, rescale=False)
    dat_sigmatot = griddata(kpc_r_cl, Msunpc2_SigmaTot, kpc_r, method='linear', fill_value=nan, rescale=False)
    # dat_sigma = griddata(kpc_r_f, Msunpc2_SigmaHI_f*g_Msun/(cm_pc)
    #                     ** 2, kpc_r, method='linear', fill_value=nan, rescale=False)
    dat_sigma = griddata(kpc_r_f, Msunpc2_SigmaHI_f, kpc_r, method='linear', fill_value=nan, rescale=False)
    ...
    molfrac = griddata(kpc_r_molfrac, MolFrac, kpc_r,
                    method='linear', fill_value=nan, rescale=False)
    dat_sigmaH2 = dat_sigma*(molfrac/((1-molfrac))) # Nieten figure 10
...
# used to import vdisp data to plots.ipynb 
# not included above since pickle file doesnt need vdisp in it
rad_data = [kpc_r_cl, kpc_r_Chemin, kpc_r_SFR, kpc_r_molfrac]
kpc_r = rad_data[np.argmin(np.array([d.size for d in rad_data]))] # kpc_r_SFR has least size
